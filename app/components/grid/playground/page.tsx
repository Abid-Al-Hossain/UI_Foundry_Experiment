...
import { DEFAULT_GRID_STATE } from "../_data/GridPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("grid-component");
  ...
  const update = <K extends keyof GridState>(key: K, value: GridState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_GRID_STATE, ...preset.state }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
