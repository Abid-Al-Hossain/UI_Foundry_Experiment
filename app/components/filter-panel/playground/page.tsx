...
import { DEFAULT_FILTERPANEL_STATE } from "../_data/FilterPanelPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("filter-panel-component");
  ...
  const update = <K extends keyof FilterPanelState>(key: K, value: FilterPanelState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_FILTERPANEL_STATE, ...(preset.state as Partial<FilterPanelState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
