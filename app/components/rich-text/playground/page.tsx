...
import { DEFAULT_RICHTEXT_STATE, RICHTEXT_PRESETS } from "../_data/RichTextPresets";
...
import { SECTIONS, type SectionId, type RichTextState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<RichTextState>(DEFAULT_RICHTEXT_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_RICHTEXT_STATE, RICHTEXT_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("rich-text-component");
  ...
  const update = <K extends keyof RichTextState>(key: K, value: RichTextState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_RICHTEXT_STATE, ...(preset.state as Partial<RichTextState>) }); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Rich Text Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
