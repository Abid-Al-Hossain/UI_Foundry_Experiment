...
import { DEFAULT_RICHTEXT_STATE } from "../_data/RichTextPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("rich-text-component");
  ...
  const update = <K extends keyof RichTextState>(key: K, value: RichTextState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_RICHTEXT_STATE, ...(preset.state as Partial<RichTextState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
