...
import { DEFAULT_BREADCRUMB_STATE, BREADCRUMB_PRESETS } from "../_data/BreadcrumbPresets";
...
export default function Page() {
  ...
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_BREADCRUMB_STATE, BREADCRUMB_PRESETS), [state]);
  ...
  const update = <K extends keyof BreadcrumbState>(key: K, value: BreadcrumbState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_BREADCRUMB_STATE, ...(preset.state as Partial<BreadcrumbState>) }); setPreviewResetKey((value) => value + 1); };
