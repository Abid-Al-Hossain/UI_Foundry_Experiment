...
import { DEFAULT_SIDEBAR_STATE } from "../_data/SidebarPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("sidebar-component");
  ...
  const update = <K extends keyof SidebarState>(key: K, value: SidebarState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_SIDEBAR_STATE, ...(preset.state as Partial<SidebarState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
