...
import { DEFAULT_SIDEBAR_STATE, SIDEBAR_PRESETS } from "../_data/SidebarPresets";
...
import { SECTIONS, type SectionId, type SidebarState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<SidebarState>(DEFAULT_SIDEBAR_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_SIDEBAR_STATE, SIDEBAR_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("sidebar-component");
  ...
  const update = <K extends keyof SidebarState>(key: K, value: SidebarState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_SIDEBAR_STATE, ...(preset.state as Partial<SidebarState>) }); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Sidebar Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
