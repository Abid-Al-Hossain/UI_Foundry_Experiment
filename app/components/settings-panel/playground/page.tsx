...
import { DEFAULT_SETTINGSPANEL_STATE } from "../_data/SettingsPanelPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("settings-panel-component");
  ...
  const update = <K extends keyof SettingsPanelState>(key: K, value: SettingsPanelState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_SETTINGSPANEL_STATE, ...(preset.state as Partial<SettingsPanelState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
