...
import { DEFAULT_SETTINGSPANEL_STATE, SETTINGSPANEL_PRESETS } from "../_data/SettingsPanelPresets";
...
import { SECTIONS, type SectionId, type SettingsPanelState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<SettingsPanelState>(DEFAULT_SETTINGSPANEL_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_SETTINGSPANEL_STATE, SETTINGSPANEL_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("settings-panel-component");
  ...
  const update = <K extends keyof SettingsPanelState>(key: K, value: SettingsPanelState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_SETTINGSPANEL_STATE, ...(preset.state as Partial<SettingsPanelState>) }); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Settings Panel Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
