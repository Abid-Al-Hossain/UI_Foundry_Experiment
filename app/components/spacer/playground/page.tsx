...
import { DEFAULT_SPACER_STATE, SPACER_PRESETS } from "../_data/SpacerPresets";
...
export default function Page() {
  ...
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_SPACER_STATE, SPACER_PRESETS), [state]);
  ...
  const update = <K extends keyof SpacerState>(key: K, value: SpacerState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_SPACER_STATE, ...(preset.state as Partial<SpacerState>) }); setPreviewResetKey((value) => value + 1); };
  const resetState = () => { setState(DEFAULT_SPACER_STATE); setPreviewResetKey((value) => value + 1); };
