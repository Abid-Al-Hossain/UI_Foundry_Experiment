...
import { DEFAULT_NAVBAR_STATE, NAVBAR_PRESETS } from "../_data/NavbarPresets";
...
import { SECTIONS, type SectionId, type NavbarState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<NavbarState>(DEFAULT_NAVBAR_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_NAVBAR_STATE, NAVBAR_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("navbar-component");
  ...
  const update = <K extends keyof NavbarState>(key: K, value: NavbarState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_NAVBAR_STATE, ...(preset.state as Partial<NavbarState>) }); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Navbar Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
