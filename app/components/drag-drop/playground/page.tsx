...
import { DEFAULT_DRAGDROP_STATE, DRAGDROP_PRESETS } from "../_data/DragDropPresets";
...
export default function Page() {
  ...
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_DRAGDROP_STATE, DRAGDROP_PRESETS), [state]);
  ...
  const update = <K extends keyof DragDropState>(key: K, value: DragDropState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_DRAGDROP_STATE, ...(preset.state as Partial<DragDropState>) }); setPreviewResetKey((value) => value + 1); };
