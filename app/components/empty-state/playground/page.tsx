...
import { DEFAULT_EMPTYSTATE_STATE } from "../_data/EmptyStatePresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("empty-state-component");
  ...
  const update = <K extends keyof EmptyStateState>(key: K, value: EmptyStateState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_EMPTYSTATE_STATE, ...(preset.state as Partial<EmptyStateState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
