...
import { DEFAULT_EMPTYSTATE_STATE, EMPTYSTATE_PRESETS } from "../_data/EmptyStatePresets";
...
import { SECTIONS, type SectionId, type EmptyStateState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<EmptyStateState>(DEFAULT_EMPTYSTATE_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_EMPTYSTATE_STATE, EMPTYSTATE_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("empty-state-component");
  ...
  const update = <K extends keyof EmptyStateState>(key: K, value: EmptyStateState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_EMPTYSTATE_STATE, ...(preset.state as Partial<EmptyStateState>) }); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Empty State Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
