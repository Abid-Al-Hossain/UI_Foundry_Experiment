...
import { DEFAULT_TABLE_STATE, TABLE_PRESETS } from "../_data/TablePresets";
...
import { SECTIONS, type SectionId, type TableState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<TableState>(DEFAULT_TABLE_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_TABLE_STATE, TABLE_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("tables-component");
  ...
  const update = <K extends keyof TableState>(key: K, value: TableState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_TABLE_STATE, ...(preset.state as Partial<TableState>) }); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Table Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
