...
import { DEFAULT_TABLE_STATE } from "../_data/TablePresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("tables-component");
  ...
  const update = <K extends keyof TableState>(key: K, value: TableState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_TABLE_STATE, ...(preset.state as Partial<TableState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
