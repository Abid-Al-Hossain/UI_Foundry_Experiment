...
import { DEFAULT_TIMELINE_STATE, TIMELINE_PRESETS } from "../_data/TimelinePresets";
...
export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<TimelineState>(DEFAULT_TIMELINE_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_TIMELINE_STATE, TIMELINE_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("timeline-component");
  ...
  const update = <K extends keyof TimelineState>(key: K, value: TimelineState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_TIMELINE_STATE, ...(preset.state as Partial<TimelineState>) }); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Timeline Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
