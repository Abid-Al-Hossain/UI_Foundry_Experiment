...
import { DEFAULT_SECTION_STATE, SECTION_PRESETS } from "../_data/SectionPresets";
...
export default function Page() {
  ...
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_SECTION_STATE, SECTION_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("section-component");
  ...
  const update = <K extends keyof SectionState>(key: K, value: SectionState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_SECTION_STATE, ...(preset.state as Partial<SectionState>) }); setPreviewResetKey((value) => value + 1); };
  const resetState = () => { setState(DEFAULT_SECTION_STATE); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Section Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
