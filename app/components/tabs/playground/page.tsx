...
import { DEFAULT_TABS_STATE, TABS_PRESETS } from "../_data/TabsPresets";
...
import { SECTIONS, type SectionId, type TabsState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<TabsState>(DEFAULT_TABS_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_TABS_STATE, TABS_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("tabs-component");
  ...
  const update = <K extends keyof TabsState>(key: K, value: TabsState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_TABS_STATE, ...(preset.state as Partial<TabsState>) }); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Tabs Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
