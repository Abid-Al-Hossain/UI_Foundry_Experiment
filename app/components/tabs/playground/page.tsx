...
import { DEFAULT_TABS_STATE } from "../_data/TabsPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("tabs-component");
  ...
  const update = <K extends keyof TabsState>(key: K, value: TabsState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_TABS_STATE, ...(preset.state as Partial<TabsState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
