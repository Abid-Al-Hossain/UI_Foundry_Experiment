...
import { DEFAULT_CHART_STATE, CHART_PRESETS } from "../_data/ChartPresets";
...
export default function Page() {
  ...
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_CHART_STATE, CHART_PRESETS), [state]);
  ...
  const update = <K extends keyof ChartState>(key: K, value: ChartState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_CHART_STATE, ...(preset.state as Partial<ChartState>) }); setPreviewResetKey((value) => value + 1); };
  const resetPreset = () => { setState(DEFAULT_CHART_STATE); setPreviewResetKey((value) => value + 1); };
