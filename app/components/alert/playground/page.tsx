...
import { DEFAULT_ALERT_STATE } from "../_data/AlertPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("alert-component");
  ...
  const update = <K extends keyof AlertState>(key: K, value: AlertState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_ALERT_STATE, ...(preset.state as Partial<AlertState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
