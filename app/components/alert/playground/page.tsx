...
import { DEFAULT_ALERT_STATE, ALERT_PRESETS } from "../_data/AlertPresets";
...
import { SECTIONS, type SectionId, type AlertState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<AlertState>(DEFAULT_ALERT_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_ALERT_STATE, ALERT_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("alert-component");
  ...
  const update = <K extends keyof AlertState>(key: K, value: AlertState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_ALERT_STATE, ...(preset.state as Partial<AlertState>) }); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Alert Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
