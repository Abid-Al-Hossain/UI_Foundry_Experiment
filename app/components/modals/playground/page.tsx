...
import { DEFAULT_MODAL_STATE } from "../_data/ModalPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("modals-component");
  ...
  const update = <K extends keyof ModalState>(key: K, value: ModalState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_MODAL_STATE, ...(preset.state as Partial<ModalState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
