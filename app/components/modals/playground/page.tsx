...
import { DEFAULT_MODAL_STATE, MODAL_PRESETS } from "../_data/ModalPresets";
...
import { SECTIONS, type SectionId, type ModalState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<ModalState>(DEFAULT_MODAL_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_MODAL_STATE, MODAL_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("modals-component");
  ...
  const update = <K extends keyof ModalState>(key: K, value: ModalState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_MODAL_STATE, ...(preset.state as Partial<ModalState>) }); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Modal Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
