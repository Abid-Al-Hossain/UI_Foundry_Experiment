...
export default function PresetsSection({ activePresetId, onApply }: Props) {
  ...
  const families = useMemo(() => ["all", ...Array.from(new Set(DATE_PICKER_PRESETS.map((preset) => preset.family)))], []);
  const variants = useMemo(() => ["all", ...Array.from(new Set(DATE_PICKER_PRESETS.map((preset) => preset.variant)))], []);
  const sizes = useMemo(() => ["all", ...Array.from(new Set(DATE_PICKER_PRESETS.map((preset) => preset.size)))], []);
  const filtered = useMemo(() => {
    const search = query.trim().toLowerCase();
    return DATE_PICKER_PRESETS.filter((preset) => {
  ...
  const surprise = () => {
    const source = filtered.length ? filtered : DATE_PICKER_PRESETS;
    onApply(source[Math.floor(Math.random() * source.length)]);
  };

  return (
    <SectionCard title="Presets" subtitle="Structured full-state presets with reset filters, result count, pagination, surprise, and applied-state highlighting.">
      ...
      <div className="grid gap-3">
        {visible.length ? visible.map((preset) => (
          ...
          <div className="rounded-2xl border p-4 text-sm" style={{ borderColor: "var(--border)", color: "var(--muted)" }}>
            No date picker presets match the current filters. Reset filters to continue.
          </div>
        )}
      </div>
