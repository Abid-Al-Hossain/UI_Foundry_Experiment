...
import { DEFAULT_ACCORDION_STATE, ACCORDION_PRESETS } from "../_data/AccordionPresets";
...
export default function Page() {
  ...
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_ACCORDION_STATE, ACCORDION_PRESETS), [state]);
  ...
  const update = <K extends keyof AccordionState>(key: K, value: AccordionState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_ACCORDION_STATE, ...(preset.state as Partial<AccordionState>) }); setPreviewResetKey((value) => value + 1); };
