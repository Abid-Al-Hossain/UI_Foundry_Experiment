...
import { DEFAULT_LAYOUTDIVIDER_STATE } from "../_data/LayoutDividerPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("layout-divider-component");
  ...
  const update = <K extends keyof LayoutDividerState>(key: K, value: LayoutDividerState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_LAYOUTDIVIDER_STATE, ...(preset.state as Partial<LayoutDividerState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
  const resetState = () => { setState(DEFAULT_LAYOUTDIVIDER_STATE); setActivePresetId(null); setPreviewResetKey((value) => value + 1); };
