...
import { DEFAULT_LAYOUTDIVIDER_STATE, LAYOUTDIVIDER_PRESETS } from "../_data/LayoutDividerPresets";
...
import { SECTIONS, type SectionId, type LayoutDividerState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<LayoutDividerState>(DEFAULT_LAYOUTDIVIDER_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_LAYOUTDIVIDER_STATE, LAYOUTDIVIDER_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("layout-divider-component");
  ...
  const update = <K extends keyof LayoutDividerState>(key: K, value: LayoutDividerState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_LAYOUTDIVIDER_STATE, ...(preset.state as Partial<LayoutDividerState>) }); setPreviewResetKey((value) => value + 1); };
  const resetState = () => { setState(DEFAULT_LAYOUTDIVIDER_STATE); setPreviewResetKey((value) => value + 1); };
  ...
  const controls = <><SectionSelector sections={SECTIONS} active={activeSection} onChange={setActiveSection} />{activeSection === "presets" && <PresetsSection activePresetId={activePresetId} onApply={applyPreset} onReset={resetState} />}{activeSection === "basics" && <BasicsSection state={state} update={update} />}{activeSection === "metadata" && <MetadataSection state={state} update={update} />}{activeSection === "structure" && <StructureSection state={state} update={update} />}{activeSection === "layout" && <LayoutSection state={state} update={update} />}{activeSection === "sizing" && <SizingSection state={state} update={update} />}{activeSection === "spacing" && <SpacingSection state={state} update={update} />}{activeSection === "surface" && <SurfaceSection state={state} update={update} />}{activeSection === "colors" && <ColorsSection state={state} update={update} />}{activeSection === "border" && <BorderSection state={state} update={update} />}{activeSection === "radius" && <RadiusSection state={state} update={update} />}{activeSection === "shadow" && <ShadowSection state={state} update={update} />}{activeSection === "typography" && <TypographySection state={state} update={update} />}{activeSection === "states" && <StatesSection state={state} update={update} />}{activeSection === "accessibility" && <AccessibilitySection state={state} update={update} />}</>;
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Layout Divider Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
