...
import { DEFAULT_CARD_STATE, CARD_PRESETS } from "../_data/CardPresets";
...
import MediaSection from "../_section/MediaSection";
import ActionsSection from "../_section/ActionsSection";
...
import { SECTIONS, type SectionId, type CardStudioState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<CardStudioState>(DEFAULT_CARD_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_CARD_STATE, CARD_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("cards-component");
  ...
  const update = <K extends keyof CardStudioState>(key: K, value: CardStudioState[K]) => {
  ...
  const controls = (
    <>
      ...
      {activeSection === "media" && <MediaSection state={state} update={update} />}
      {activeSection === "actions" && <ActionsSection state={state} update={update} />}
      ...
      {activeSection === "hover" && <HoverSection state={state} update={update} />}
  ...
  return (
    <AppShell contentOverflow="hidden">
      <PlaygroundLayout title="Card Studio" headerActions={headerActions} controls={controls} preview={output} />
