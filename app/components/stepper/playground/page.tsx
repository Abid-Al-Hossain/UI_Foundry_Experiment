...
import { DEFAULT_STEPPER_STATE, STEPPER_PRESETS } from "../_data/StepperPresets";
...
export default function Page() {
  ...
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_STEPPER_STATE, STEPPER_PRESETS), [state]);
  ...
  const update = <K extends keyof StepperState>(key: K, value: StepperState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_STEPPER_STATE, ...(preset.state as Partial<StepperState>) }); setPreviewResetKey((value) => value + 1); };
