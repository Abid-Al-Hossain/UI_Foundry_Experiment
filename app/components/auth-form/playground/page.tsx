...
import { DEFAULT_AUTHFORM_STATE, AUTHFORM_PRESETS } from "../_data/AuthFormPresets";
...
export default function Page() {
  ...
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_AUTHFORM_STATE, AUTHFORM_PRESETS), [state]);
  ...
  const update = <K extends keyof AuthFormState>(key: K, value: AuthFormState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_AUTHFORM_STATE, ...(preset.state as Partial<AuthFormState>) }); setPreviewResetKey((value) => value + 1); };
