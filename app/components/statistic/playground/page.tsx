...
import { DEFAULT_STATISTIC_STATE, STATISTIC_PRESETS } from "../_data/StatisticPresets";
...
export default function Page() {
  ...
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_STATISTIC_STATE, STATISTIC_PRESETS), [state]);
  ...
  const update = <K extends keyof StatisticState>(key: K, value: StatisticState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_STATISTIC_STATE, ...(preset.state as Partial<StatisticState>) }); setPreviewResetKey((value) => value + 1); };
