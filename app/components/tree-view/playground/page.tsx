...
import { DEFAULT_TREEVIEW_STATE, TREEVIEW_PRESETS } from "../_data/TreeViewPresets";
...
import { SECTIONS, type SectionId, type TreeViewState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<TreeViewState>(DEFAULT_TREEVIEW_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_TREEVIEW_STATE, TREEVIEW_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("tree-view-component");
  ...
  const update = <K extends keyof TreeViewState>(key: K, value: TreeViewState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_TREEVIEW_STATE, ...(preset.state as Partial<TreeViewState>) }); setPreviewResetKey((value) => value + 1); };
  const exportPayload = useMemo(() => buildExportPayload(state, downloadName), [downloadName, state]);
  const preview = useMemo(
    () => (
      <LivePreview
        key={`${previewResetKey}:${state.id}:${state.itemCount}:${state.depth}:${state.selectionMode}:${state.expandedCount}`}
        state={state}
      />
    ),
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Tree View Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
