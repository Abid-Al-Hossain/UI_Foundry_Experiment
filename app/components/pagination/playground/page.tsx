...
import { DEFAULT_PAGINATION_STATE, PAGINATION_PRESETS } from "../_data/PaginationPresets";
...
import { SECTIONS, type SectionId, type PaginationState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<PaginationState>(DEFAULT_PAGINATION_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_PAGINATION_STATE, PAGINATION_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("pagination-component");
  ...
  const update = <K extends keyof PaginationState>(key: K, value: PaginationState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_PAGINATION_STATE, ...(preset.state as Partial<PaginationState>) }); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Pagination Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
