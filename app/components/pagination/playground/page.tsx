...
import { DEFAULT_PAGINATION_STATE } from "../_data/PaginationPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("pagination-component");
  ...
  const update = <K extends keyof PaginationState>(key: K, value: PaginationState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_PAGINATION_STATE, ...(preset.state as Partial<PaginationState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
