...
import { DEFAULT_DROPDOWNMENU_STATE } from "../_data/DropdownMenuPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("dropdown-menu-component");
  ...
  const update = <K extends keyof DropdownMenuState>(key: K, value: DropdownMenuState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_DROPDOWNMENU_STATE, ...(preset.state as Partial<DropdownMenuState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
