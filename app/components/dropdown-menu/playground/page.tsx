...
import { DEFAULT_DROPDOWNMENU_STATE, DROPDOWNMENU_PRESETS } from "../_data/DropdownMenuPresets";
...
import { SECTIONS, type SectionId, type DropdownMenuState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<DropdownMenuState>(DEFAULT_DROPDOWNMENU_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_DROPDOWNMENU_STATE, DROPDOWNMENU_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("dropdown-menu-component");
  ...
  const update = <K extends keyof DropdownMenuState>(key: K, value: DropdownMenuState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_DROPDOWNMENU_STATE, ...(preset.state as Partial<DropdownMenuState>) }); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Dropdown Menu Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
