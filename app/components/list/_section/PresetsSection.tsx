...
export default function PresetsSection({ activePresetId, onApply }: { activePresetId: string | null; onApply: (preset: StudioPreset) => void }) {
  ...
  const families = useMemo(() => ["all", ...Array.from(new Set(LIST_PRESETS.map((preset) => preset.family)))], []);
  const sizes = useMemo(() => ["all", ...Array.from(new Set(LIST_PRESETS.map((preset) => preset.size)))], []);
  const filtered = LIST_PRESETS.filter((preset) => [preset.family, preset.archetype, preset.variant, preset.size, ...preset.tags].join(" ").toLowerCase().includes(query.toLowerCase()) && (family === "all" || preset.family === family) && (size === "all" || preset.size === size));
  const source = filtered.length ? filtered : LIST_PRESETS;
      ...
      <div data-testid="preset-browser" data-active-preset-id={activePresetId ?? ""} className="grid gap-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p data-testid="preset-result-count" className="text-sm" style={{ color: "var(--muted)" }}>
            Showing {filtered.length} of {LIST_PRESETS.length} presets
          </p>
