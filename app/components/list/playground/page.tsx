...
import { DEFAULT_LIST_STATE, LIST_PRESETS } from "../_data/ListPresets";
...
import { SECTIONS, type SectionId, type ListState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<ListState>(DEFAULT_LIST_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_LIST_STATE, LIST_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("list-component");
  ...
  const update = <K extends keyof ListState>(key: K, value: ListState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_LIST_STATE, ...(preset.state as Partial<ListState>) }); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="List Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
