...
import { DEFAULT_LIST_STATE } from "../_data/ListPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("list-component");
  ...
  const update = <K extends keyof ListState>(key: K, value: ListState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_LIST_STATE, ...(preset.state as Partial<ListState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
