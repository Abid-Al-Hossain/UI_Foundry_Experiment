...
import { DEFAULT_DRAWER_STATE } from "../_data/DrawerPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("drawer-component");
  ...
  const update = <K extends keyof DrawerState>(key: K, value: DrawerState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_DRAWER_STATE, ...(preset.state as Partial<DrawerState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
