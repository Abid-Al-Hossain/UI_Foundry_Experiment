...
import { DEFAULT_HEADER_STATE, HEADER_PRESETS } from "../_data/HeaderPresets";
...
export default function Page() {
  ...
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_HEADER_STATE, HEADER_PRESETS), [state]);
  ...
  const update = <K extends keyof HeaderState>(key: K, value: HeaderState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_HEADER_STATE, ...(preset.state as Partial<HeaderState>) }); setPreviewResetKey((value) => value + 1); };
