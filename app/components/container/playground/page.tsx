...
import { DEFAULT_CONTAINER_STATE, CONTAINER_PRESETS } from "../_data/ContainerPresets";
...
import { SECTIONS, type SectionId, type ContainerState, type StudioPreset } from "../types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<ContainerState>(DEFAULT_CONTAINER_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_CONTAINER_STATE, CONTAINER_PRESETS), [state]);
  const [downloadName, setDownloadName] = useState("container-component");
  ...
  const update = <K extends keyof ContainerState>(key: K, value: ContainerState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_CONTAINER_STATE, ...(preset.state as Partial<ContainerState>) }); setPreviewResetKey((value) => value + 1); };
  const resetState = () => { setState(DEFAULT_CONTAINER_STATE); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Container Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
