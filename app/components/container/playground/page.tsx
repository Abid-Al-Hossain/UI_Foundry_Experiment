...
import { DEFAULT_CONTAINER_STATE } from "../_data/ContainerPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("container-component");
  ...
  const update = <K extends keyof ContainerState>(key: K, value: ContainerState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_CONTAINER_STATE, ...(preset.state as Partial<ContainerState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
  const resetState = () => { setState(DEFAULT_CONTAINER_STATE); setActivePresetId(null); setPreviewResetKey((value) => value + 1); };
