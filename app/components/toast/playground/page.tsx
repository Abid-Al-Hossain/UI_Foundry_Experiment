...
import { DEFAULT_TOAST_STATE, TOAST_PRESETS } from "../_data/ToastPresets";
...
export default function Page() {
  ...
  const activePresetId = useMemo(() => findActivePresetId(state, DEFAULT_TOAST_STATE, TOAST_PRESETS), [state]);
  ...
  const update = <K extends keyof ToastState>(key: K, value: ToastState[K]) => { setState((current) => ({ ...current, [key]: value })); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_TOAST_STATE, ...(preset.state as Partial<ToastState>) }); setPreviewResetKey((value) => value + 1); };
