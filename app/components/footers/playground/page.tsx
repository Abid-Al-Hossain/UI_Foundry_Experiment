...
import { DEFAULT_FOOTER_STATE } from "../_data/FooterPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("footers-component");
  ...
  const update = <K extends keyof FooterState>(key: K, value: FooterState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_FOOTER_STATE, ...(preset.state as Partial<FooterState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
