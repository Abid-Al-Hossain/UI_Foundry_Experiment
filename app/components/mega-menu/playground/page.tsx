...
import { DEFAULT_MEGAMENU_STATE } from "../_data/MegaMenuPresets";
...
export default function Page() {
  ...
  const [downloadName] = useState("mega-menu-component");
  ...
  const update = <K extends keyof MegaMenuState>(key: K, value: MegaMenuState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_MEGAMENU_STATE, ...(preset.state as Partial<MegaMenuState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
